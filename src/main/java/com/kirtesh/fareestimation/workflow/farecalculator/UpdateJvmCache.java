...
public class UpdateJvmCache implements WorkflowStep<EventMetadata> {

    private final static Logger logger = LogManager.getLogger(FareEstimation.class);

    private String name = "UpdateJvmCache";
    ...
    public WorkflowStepResponse<EventMetadata> process(EventMetadata metadata) {
        ...
        metadata.setNewRideEvent(false);
        JvmPreviousEvent.resetCache();
        JvmPreviousEvent.setEventMetadata(metadata);
